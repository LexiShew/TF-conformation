"""fig7_abs_pearson_groups — regenerates fig7_abs_pearson_groups.png
...
def six_group_vfig(metric, title, fname, ylim, ylabel):
        ...
        for xi,tf in zip(xs,order9):
            if dna=='relax' and tf=='csl' and len(grp6(tf,dna,'baseline','all',metric))==0:
                ax.text(xi,np.mean(ylim),'n.d.\n(no relaxed\nDNA run)',color=GREEN[2],fontsize=6,
                        va='center',ha='center',style='italic'); continue
            for (arm,sub),dx in zip(specs,offs):
                v = grp6(tf,dna,arm,sub,metric)
                if len(v)==0: continue
                si = subsets.index(sub); fc = GREY[si] if arm=='baseline' else augcols[si]
                ...
                for p in bp['boxes']: p.set_facecolor(fc);p.set_edgecolor('0.35');p.set_linewidth(0.5)
    ...
    axes[1].set_xticks(xs); axes[1].set_xticklabels([lab2[t] for t in order9],rotation=0,ha='center',fontsize=7.5)
...
figP = six_group_vfig('mean_pearsonr',
    "Per-entry accuracy: baseline vs augmented, split by entry subset (all / other-family / same-family)",
    'fig7_abs_pearson_groups.png',(0.15,0.98),'Pearson r per entry  (higher = better ↑)')

for ax in figP.axes:
    ax.yaxis.set_label_coords(-0.055, 0.5)
figP.subplots_adjust(left=0.10)
figP.subplots_adjust(hspace=0.22)
figP.savefig('fig7_abs_pearson_groups.png',dpi=300,bbox_inches='tight')
