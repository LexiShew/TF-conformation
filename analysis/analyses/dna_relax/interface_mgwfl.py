...
Run under `deeppbs` conda env from the TF-conformation repo root:
    python analysis/analyses/dna_relax/interface_mgwfl.py
...
def iface_levels(tf,pid,contacts,chains):
    cf=glob.glob(f"analysis/analyses/dna_relax/pycurves/{tf}/crystal_legacy.json")
...
if __name__=="__main__":
    ...
    for tf,pid in PILOTS.items():
        try:
            ...
            froz=ensemble_fl(sorted(glob.glob(f"analysis/analyses/dna_relax/pycurves/{tf}/frozen_state_*_legacy.json")))
            relax=ensemble_fl(sorted(glob.glob(f"analysis/analyses/dna_relax/pycurves/{tf}/relaxed_state_*_legacy.json")))
    ...
    import csv
    with open("analysis/analyses/dna_relax/data/iface_mgwfl_vs_accuracy.csv","w",newline="") as fh:
        w=csv.DictWriter(fh,fieldnames=sorted({k for r in rows for k in r})); w.writeheader(); w.writerows(rows)
    print("wrote analysis/analyses/dna_relax/data/iface_mgwfl_vs_accuracy.csv", len(rows),"pilots")
