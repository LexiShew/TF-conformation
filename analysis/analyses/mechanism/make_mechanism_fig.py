...
# Colour comes from palette.py at the repo root -- the single source of truth for the
# project (GREY = baseline/reference, TEAL = augmented frozen DNA, GREEN = augmented
# relaxed DNA, ALARM = negative annotation ONLY, never a data series).
# An earlier version of this script invented its own hues and used a pink for negative
# BARS, which both broke the per-entity threading and used the reserved alarm hue as a
# data series. Do not reintroduce literal hex here.
import sys
sys.path.insert(0, str(ROOT))
from palette import GREY, TEAL, GREEN, ALARM, apply_style   # noqa: E402

C_SELF = TEAL       # own-family effect  (the frozen-DNA augmented thread)
C_OTHER = "#C7CCD1" # other-family background (light neutral, not an entity)
GREY_TXT = "#4A4A4A"

apply_style()   # canonical rc ladder from palette.py
...
def panel_b(ax, df):
    ...
    y = np.arange(len(d))
    # Sign is encoded teal (own-family favoured) vs neutral grey (not favoured).
    # ALARM is reserved for annotation and is NOT used as a bar colour.
    colors = [TEAL if v > 0 else GREY for v in d.contrast]
    ax.axvline(0, color=GREY, lw=0.8, zorder=1)
    ...
    for i, (tf, row) in enumerate(d.iterrows()):
        if row.p < 0.05:
            ax.text(row.contrast + 0.006, i, f"p={row.p:.3f}",
                    va="center", ha="left", fontsize=6, color=GREY_TXT)
...
def panel_c(ax, df):
    ...
    ax.scatter(df.base_self_P, df.self_dP_mean, s=34, color=TEAL,
...
def panel_d(ax, df):
    ...
    ax.scatter(d1.froz_bend_iqr, d1.self_dP_mean, s=34, color=TEAL,
               edgecolor="white", linewidth=0.5, zorder=3,
    ...
    ax.plot(xs, m * xs + b, color=TEAL, lw=1.0, alpha=0.55, zorder=2)
...
def main():
    ...
    fig.text(0.01, 0.005,
             ...
             fontsize=6, color=GREY_TXT, ha="left")
