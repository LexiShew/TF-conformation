#!/bin/bash
# Pilot config: ENGRAILED — DNA-relaxation variant (Tier 1).
# homeodomain (induced-fit test)
#
# Identical to engrailed.sh but opts in to the DNA-relaxation pipeline. TF_NAME is
# inherited from engrailed.sh so Stage 1 (BioEmu library) and Stage 2 (docked frames)
...
# Inherit the full engrailed config (TF_NAME, PDB_ID, chains, ramp, N_FRAMES, fold...).
_here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck disable=SC1091
source "${_here}/engrailed.sh"

# --- DNA-relaxation opt-in (this is the whole delta vs engrailed.sh) ---
