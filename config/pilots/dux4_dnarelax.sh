#!/bin/bash
# Pilot config: DUX4 — DNA-relaxation variant (Tier 1).
# double homeodomain (rigid-ish control)
#
# Identical to dux4.sh but opts in to the DNA-relaxation pipeline. TF_NAME is
# inherited from dux4.sh so Stage 1 (BioEmu library) and Stage 2 (docked frames)
...
# Inherit the full dux4 config (TF_NAME, PDB_ID, chains, ramp, N_FRAMES, fold...).
_here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck disable=SC1091
source "${_here}/dux4.sh"

# --- DNA-relaxation opt-in (this is the whole delta vs dux4.sh) ---
