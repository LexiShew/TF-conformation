#!/bin/bash
# Pilot config: LEF1 — DNA-relaxation variant (Tier 1).
# HMG-box, sharp DNA bend (induced-fit test)
#
# Identical to lef1.sh but opts in to the DNA-relaxation pipeline. TF_NAME is
# inherited from lef1.sh so Stage 1 (BioEmu library) and Stage 2 (docked frames)
...
# Inherit the full lef1 config (TF_NAME, PDB_ID, chains, ramp, N_FRAMES, fold...).
_here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck disable=SC1091
source "${_here}/lef1.sh"

# --- DNA-relaxation opt-in (this is the whole delta vs lef1.sh) ---
