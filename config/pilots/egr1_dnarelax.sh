#!/bin/bash
# Pilot config: EGR1 — DNA-relaxation variant (Tier 1).
# C2H2 zinc-finger (rigid control)
#
# Identical to egr1.sh but opts in to the DNA-relaxation pipeline. TF_NAME is
# inherited from egr1.sh so Stage 1 (BioEmu library) and Stage 2 (docked frames)
...
# Inherit the full egr1 config (TF_NAME, PDB_ID, chains, ramp, N_FRAMES, fold...).
_here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck disable=SC1091
source "${_here}/egr1.sh"

# --- DNA-relaxation opt-in (this is the whole delta vs egr1.sh) ---
