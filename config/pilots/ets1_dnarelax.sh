#!/bin/bash
# Pilot config: ETS1 — DNA-relaxation variant (Tier 1).
# winged-helix ETS (rigid control)
#
# Identical to ets1.sh but opts in to the DNA-relaxation pipeline. TF_NAME is
# inherited from ets1.sh so Stage 1 (BioEmu library) and Stage 2 (docked frames)
...
# Inherit the full ets1 config (TF_NAME, PDB_ID, chains, ramp, N_FRAMES, fold...).
_here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# shellcheck disable=SC1091
source "${_here}/ets1.sh"

# --- DNA-relaxation opt-in (this is the whole delta vs ets1.sh) ---
